...
public class HistoryDetailActivity extends BaseActivity implements LoaderManager.LoaderCallbacks<Cursor>{
    ...
    private class QHandler extends AsyncQueryHandler {
        ...
        private final int DELETE_ENTRY = 6;

        @Override
        protected void onDeleteComplete(int token, Object cookie, int result) {
            super.onDeleteComplete(token, cookie, result);
            if (token == DELETE_ENTRY) {
                finish();
            }
        }
...
/*        mQHandler.startQuery(OVERLAP_CHECK,
    ...
    private void deleteDiaryEntry() {
        // 执行删除日记条目的逻辑，可以使用 AsyncQueryHandler 进行异步删除
        mQHandler.startDelete(R.id.action_edit_delete, null, ContentUris.withAppendedId(ActivityDiaryContract.Diary.CONTENT_URI, diaryEntryID),
                null, null);
