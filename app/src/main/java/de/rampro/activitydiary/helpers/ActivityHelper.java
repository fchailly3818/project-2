    ...
    protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
        ...
        if ((cursor != null) && cursor.moveToFirst()) {
            if(token == QUERY_ALL_ACTIVITIES) {
                ActivityDiaryContentProvider tmp = new ActivityDiaryContentProvider();
                synchronized (this) {
                    activities.clear();
                    ...
                    while (!cursor.isAfterLast()) {
                        DiaryActivity act = new DiaryActivity(cursor.getInt(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity._ID)),
                                ...
                                //cursor.getInt(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity._ID)) % 5
                                cursor.getInt(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity.PARENT))
                        );
                ...
                act.setMtype(cursor.getInt(cursor.getColumnIndex(ActivityDiaryContract.DiaryActivity.PARENT)));
                //act.setMtype(0);
                for(DataChangedListener listener : mDataChangeListeners) {
